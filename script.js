let stage, underlayLayer, guideLayer, wireLayer, electricalLayer, controlLayer, measureLayer;
...
let measureMode = 'ruler'; // 'ruler' or 'calibrate'

const SNAP_THRESHOLD = 8;
const MAX_SNAP_DISTANCE = 160;

const WIRE_HIT_WIDTH = 12;
const COMPONENT_HIT_RADIUS = 18;
...
function initStage() {
  ...
  stage.on('mousedown touchstart', handleMeasureStart);
  stage.on('mousemove touchmove', handleMeasureMove);
  stage.on('mouseup touchend', handleMeasureEnd);
}
...
function checkAlignmentSnap(candidateX, candidateY, skipNodeId = null, currentType = null) {
  ...
  const currentSnapThreshold = SNAP_THRESHOLD / stage.scaleX();
  const currentMaxSnapDist = MAX_SNAP_DISTANCE / stage.scaleX();
  ...
  comps.forEach(node => {
    ...
    if (straightLineDist > currentMaxSnapDist) return;

    const diffX = Math.abs(node.x() - candidateX);
    if (diffX < currentSnapThreshold && straightLineDist < bestDistX) {
      snappedX = node.x();
      lineX = node.x();
    ...
    if (diffY < currentSnapThreshold && straightLineDist < bestDistY) {
      snappedY = node.y();
      lineY = node.y();
...
function renderGuideLines(lineX, lineY) {
  ...
  if (lineX !== null) {
    guideLayer.add(new Konva.Line({
      ...
      stroke: '#06b6d4', strokeWidth: 1.5, dash: [6, 4], listening: false
    }));
  }
  if (lineY !== null) {
    guideLayer.add(new Konva.Line({
...
function selectComponentFromLibrary(type) {
  ...
  document.getElementById('toolMeasure').classList.remove('selected');
  document.getElementById('toolMeasure').style.background = '#0284c7';
...
function selectPointerTool(isUserAction = true) {
  ...
  if (measureStart && measureCurrent) renderMeasureOverlay(false);
  if (isUserAction) dismissWelcome();
  if (!isWelcomeActive) updateModeIndicatorUI();
}
...
function updateModeIndicatorUI() {
  ...
  } else if (currentComponent === 'measure') {
    indicator.style.background = 'rgba(2, 132, 199, 0.95)';
    indicator.style.borderColor = '#0ea5e9';
    indicator.innerHTML = measureMode === 'calibrate'
      ? `🟩 Calibrate Scale <span class="mode-subtext">— Drag & snap to set baseline</span>`
      : `📏 Drop Ruler <span class="mode-subtext">— Drag & snap to drop dimension lines</span>`;
...
function setupPinchAndPan() {
  ...
  content.addEventListener('touchstart', (e) => {
    if (e.touches.length > 1) {
      ...
      if (isMeasuring) resetTapeMeasure();
    }
  }, { passive: true });
  ...
  content.addEventListener('touchmove', (e) => {
    ...
    if (touch1 && touch2) {
      ...
      const newScale = Math.max(0.2, Math.min(oldScale * distRatio, 24));
...
function resetPlanVisuals() {
  ...
  document.getElementById('planOpacity').value = 100;
  document.getElementById('lblContrast').innerText = 50;
  document.getElementById('lblBrightness').innerText = 50;
  document.getElementById('lblOpacity').innerText = 100;
  updatePlanVisuals();
  changePlanOpacity(100);
}
...
function processImageFile(file) {
  ...
  if (file.type === 'application/pdf') {
    ...
    const reader = new FileReader();
    reader.onload = (e) => { loadPlanImageFromUrl(e.target.result); };
...
function loadPlanImageFromUrl(dataUrl) {
  ...
  img.onload = () => {
    ...
    planImageNode = new Konva.Image({
      ...
      image: img, width: fitW, height: fitH, opacity: 1.0, draggable: false, name: 'planImage'
    });
...
function getComponentScale() {
  const targetSymbolPx = scalePixelsPerMeter * 0.3;
  const scaleFactor = Math.max(0.2, Math.min(targetSymbolPx / 24, 2.5));
  return getDeviceScale() * scaleFactor;
}
...
function createSymbol(type, x, y, id) {
  ...
  group.add(new Konva.Circle({ radius: 18, fill: 'transparent', hitStrokeWidth: 0 }));
  ...
  if (type === 'downlight') {
    ...
    for (let i = -10; i <= 10; i += 5) { group.add(new Konva.Circle({ radius: 1.5, x: i, y: 0, fill: '#ca8a04' })); }
...
function createWire(fromId, toId, wireId = null, recordHistory = true, type = null, routingStyle = null, customMidX = null, customMidY = null) {
  ...
  // STROKE SCALE DISABLED: Prevents wires blowing out on zoom
...
function showWireControlHandle(w) {
  ...
  const handleScale = Math.min(2.0, 1 / stage.scaleX());
...
/* ==========================================================================
   DIGITAL TAPE MEASURE ENGINE (Popover Rulers, Snapping & Clamping)
   ========================================================================== */
...
function startMeasureMode(mode) {
  if (mode === 'ruler' && !isPlanCalibrated) {
    alert("Please calibrate the plan scale first before dropping dimension lines.");
    mode = 'calibrate';
  }
...
function clearAllRulers() {
  ...
  if (!confirm("Clear all dimension lines from the plan?")) return;
  rulers.forEach(r => r.destroy());
  electricalLayer.batchDraw();
  updateStatus();
}
...
function getStraightenedMeasurePoint(p1, p2) {
  const dx = p2.x - p1.x;
  const dy = p2.y - p1.y;
  const angle = Math.abs(Math.atan2(dy, dx) * (180 / Math.PI));
  // 3-DEGREE SNAP: Highly precise for free diagonal measuring
  if (angle < 3 || angle > 177) return { x: p2.x, y: p1.y };
  if (Math.abs(angle - 90) < 3) return { x: p1.x, y: p2.y };
  return p2;
}

function resetTapeMeasure() {
  isMeasuring = false;
  measureStart = null;
  measureCurrent = null;
  if (measureLayer) {
...
function handleMeasureStart(e) {
  ...
  if (pos) {
    ...
    measureStart = pos;
    measureCurrent = pos;
    renderMeasureOverlay(false);
  }
}

function handleMeasureMove(e) {
  if (multiTouchDetected || (e.evt && e.evt.touches && e.evt.touches.length > 1)) {
    if (isMeasuring) resetTapeMeasure();
    return;
  }
  if (!isMeasuring || currentComponent !== 'measure') return;

  const pos = getRelativePointerPosition(stage);
  if (pos) {
    measureCurrent = getStraightenedMeasurePoint(measureStart, pos);
    renderMeasureOverlay(false);
  }
}
...
function handleMeasureEnd(e) {
  ...
  const screenDist = distPx * stage.scaleX();
...
function createDraggableMeasureHandle(x, y, isStartHandle, reverseScale, lineNode, badgeGroup) {
  ...
  const vLine = new Konva.Line({ points: [0, -size, 0, size], stroke: '#38bdf8', strokeWidth: 1 * reverseScale });

  group.add(touchTarget, centerDot, hLine, vLine);

  group.on('dragmove', () => {
    let candidateX = group.x();
    let candidateY = group.y();

    const snap = checkAlignmentSnap(candidateX, candidateY);
    if (snap.snappedX !== candidateX || snap.snappedY !== candidateY) {
      candidateX = snap.snappedX;
      candidateY = snap.snappedY;
    } else {
      const otherPoint = isStartHandle ? measureCurrent : measureStart;
      const straightened = getStraightenedMeasurePoint(otherPoint, { x: candidateX, y: candidateY });
      candidateX = straightened.x;
      candidateY = straightened.y;
    }

    group.position({ x: candidateX, y: candidateY });
  ...
  group.on('dragend', () => { renderMeasureOverlay(true); });
  return group;
}
...
function renderMeasureOverlay(interactive = false) {
  measureLayer.destroyChildren();
  if (!measureStart || !measureCurrent) return;

  const trueReverseScale = 1 / stage.scaleX();
  const reverseScale = Math.min(1.5, trueReverseScale);

  const line = new Konva.Line({
    points: [measureStart.x, measureStart.y, measureCurrent.x, measureCurrent.y],
    stroke: '#0ea5e9', strokeWidth: 2 * trueReverseScale, dash: [6 * trueReverseScale, 4 * trueReverseScale], listening: false
  });
  ...
  const distBg = new Konva.Rect({
    width: distText.width(), height: distText.height(), fill: '#0284c7', cornerRadius: 4 * reverseScale
  });

  badgeGroup.add(distBg, distText);
  let totalBadgeW = distText.width();

  if (interactive) {
    let btnOffsetX = distText.width() + (6 * reverseScale);

    const actionBtn = new Konva.Group({ x: btnOffsetX, y: 0 });
    const btnString = measureMode === 'calibrate' ? 'Set Scale' : '✓ Drop Ruler';
    const btnColor = measureMode === 'calibrate' ? '#10b981' : '#0284c7';
    ...
    badgeGroup.add(actionBtn);
    btnOffsetX += actionText.width() + (6 * reverseScale);

    const resetBtn = new Konva.Group({ x: btnOffsetX, y: 0 });
    const resetText = new Konva.Text({ text: '✕', fontSize: 13 * reverseScale, fontFamily: '-apple-system, sans-serif', fontStyle: 'bold', fill: '#ffffff', padding: 5 * reverseScale });
    const resetBg = new Konva.Rect({ width: resetText.width(), height: resetText.height(), fill: '#ef4444', cornerRadius: 4 * reverseScale });
    resetBtn.add(resetBg, resetText);
    resetBtn.on('click tap', (e) => { e.cancelBubble = true; resetTapeMeasure(); });
    badgeGroup.add(resetBtn);

    totalBadgeW = btnOffsetX + resetText.width();

    const handleStart = createDraggableMeasureHandle(measureStart.x, measureStart.y, true, reverseScale, line, badgeGroup);
    const handleEnd = createDraggableMeasureHandle(measureCurrent.x, measureCurrent.y, false, reverseScale, line, badgeGroup);
    measureLayer.add(handleStart, handleEnd);
  }

  badgeGroup.offsetX(totalBadgeW / 2);
  measureLayer.add(badgeGroup);
  measureLayer.batchDraw();
}
...
function handleStageTap(e) {
  ...
  if (e.target === stage || e.target.hasName('planImage') || e.target.hasName('scale-stamp')) {
    clearWireControlHandle();
    closeClearPopover();
    closeMeasurePopover();
  }
...
const CORNER_SNAP_DIST = 32;
const T_SNAP_DIST = 18;
const CAD_ALIGN_THRESHOLD = 18;
let studioAlignGuides = []; // Stores active CAD leveling extension lines
...
// Grid snap removed in favor of smart CAD corner alignment
...
function getStudioCanvasPoint(e) {
  ...
  // 3. Smart CAD Alignment & Leveling Guides against other corners
  studioAlignGuides = [];
  if (!snapped && (studioTool === 'wall' || studioTool === 'room')) {
    const alignThreshold = CAD_ALIGN_THRESHOLD / studioScale;
    let alignedX = false;
    let alignedY = false;

    for (const l of studioLines) {
      const endpoints = [{ x: l.x1, y: l.y1 }, { x: l.x2, y: l.y2 }];
      for (const ep of endpoints) {
        // Horizontal level alignment (shares ep.y)
        if (!alignedY && Math.abs(y - ep.y) < alignThreshold) {
          y = ep.y;
          alignedY = true;
          snapped = true; // Prevents grid-snap tug-of-war
          studioAlignGuides.push({ type: 'horizontal', y: ep.y, sourceX: ep.x, sourceY: ep.y });
        }
        // Vertical plumb alignment (shares ep.x)
        if (!alignedX && Math.abs(x - ep.x) < alignThreshold) {
          x = ep.x;
          alignedX = true;
          snapped = true; // Prevents grid-snap tug-of-war
          studioAlignGuides.push({ type: 'vertical', x: ep.x, sourceX: ep.x, sourceY: ep.y });
        }
      }
    }
  }

  // 4. Background Grid Snap (only when not locked onto a corner or CAD guide)
...
function handleStudioPointerDown(e) {
  ...
  if (studioTool === 'select' && selectedStudioEntity) {
    if (selectedStudioEntity.type === 'opening') {
      ...
      const hitDist = 24 / studioScale;
      if (Math.hypot(pt.x - w.x1, pt.y - w.y1) < hitDist) {
        selectedStudioEntity.isDraggingVertex = true;
        selectedStudioEntity.dragVertex = 1; // Dragging (x1, y1)
        selectedStudioEntity.origX = w.x1;
        selectedStudioEntity.origY = w.y1;
      } else if (Math.hypot(pt.x - w.x2, pt.y - w.y2) < hitDist) {
        selectedStudioEntity.isDraggingVertex = true;
        selectedStudioEntity.dragVertex = 2; // Dragging (x2, y2)
        selectedStudioEntity.origX = w.x2;
        selectedStudioEntity.origY = w.y2;
      }
    }
  }
...
function handleStudioPointerMove(e) {
  ...
  if (studioTool === 'wall') {
  ...
  } else if (studioTool === 'select' && selectedStudioEntity) {
    if (selectedStudioEntity.isDragging && selectedStudioEntity.type === 'opening') {
      const op = studioOpenings[selectedStudioEntity.index];
    ...
    } else if (selectedStudioEntity.isDraggingVertex && selectedStudioEntity.type === 'wall') {
      const w = studioLines[selectedStudioEntity.index];
      const anchor = (selectedStudioEntity.dragVertex === 1) ? { x: w.x2, y: w.y2 } : { x: w.x1, y: w.y1 };
      const aligned = autoStraightenOrthogonal(anchor, pt);

      const oldX = (selectedStudioEntity.dragVertex === 1) ? w.x1 : w.x2;
      const oldY = (selectedStudioEntity.dragVertex === 1) ? w.y1 : w.y2;

      // Update all connected wall segments sharing this joint
      studioLines.forEach(other => {
        if (Math.hypot(other.x1 - oldX, other.y1 - oldY) < 1) { other.x1 = aligned.x; other.y1 = aligned.y; }
        if (Math.hypot(other.x2 - oldX, other.y2 - oldY) < 1) { other.x2 = aligned.x; other.y2 = aligned.y; }
      });
    }
  }
...
function handleStudioPointerUp(e) {
  ...
  if (selectedStudioEntity && (selectedStudioEntity.isDragging || selectedStudioEntity.isDraggingVertex)) {
    selectedStudioEntity.isDragging = false;
    selectedStudioEntity.isDraggingVertex = false;
...
function showEditHud() {
  ...
  let header = hud.querySelector('.studio-hud-header');
  if (!header) {
    header = document.createElement('div');
    header.className = 'studio-hud-header';
    hud.insertBefore(header, actions);

    const titleWrapper = document.createElement('div');
    titleWrapper.className = 'studio-hud-drag-title';
    titleWrapper.innerHTML = `<span class="studio-drag-grip">⠿</span>`;
    titleWrapper.appendChild(title);
    header.appendChild(titleWrapper);

    // Pointer drag listeners for moving the HUD out of the way
    ...
    header.addEventListener('pointerdown', (e) => {
      ...
      // Detach center alignment transform so direct pixel coordinates work
    ...
    header.addEventListener('pointermove', (e) => {
      ...
      const newLeft = Math.max(10, Math.min(window.innerWidth - hud.offsetWidth - 10, hudDragState.initialLeft + dx));
      const newTop = Math.max(10, Math.min(window.innerHeight - hud.offsetHeight - 10, hudDragState.initialTop + dy));
    ...
    header.addEventListener('pointerup', stopHudDrag);
    header.addEventListener('pointercancel', stopHudDrag);
  }
  ...
  if (selectedStudioEntity.type === 'opening') {
    const op = studioOpenings[selectedStudioEntity.index];
    if (op.type === 'door') {
      title.innerText = '🚪 Door Options';
      actionButtons = `
        <button class="btn-hud" onclick="toggleDoorSwing()">🔄 Toggle Swing</button>
        <button class="btn-hud" onclick="toggleDoorHinge()">🔀 Toggle Hinge</button>
        <button class="btn-hud" onclick="deleteSelectedStudioEntity()" style="color:#f87171;">🗑️ Delete Door</button>
      `;
    } else if (op.type === 'slider') {
      title.innerText = '🪟 Slider Options';
      actionButtons = `
        <button class="btn-hud" onclick="toggleDoorSwing()">🔄 Reverse Track</button>
        <button class="btn-hud" onclick="toggleDoorHinge()">🔀 Flip Fixed Panel</button>
        <button class="btn-hud" onclick="deleteSelectedStudioEntity()" style="color:#f87171;">🗑️ Delete Slider</button>
      `;
    } else if (op.type === 'window') {
      title.innerText = '🪟 Window Options';
      actionButtons = `
        <button class="btn-hud" onclick="cycleWindowWidth()">📏 Change Width</button>
        <button class="btn-hud" onclick="deleteSelectedStudioEntity()" style="color:#f87171;">🗑️ Delete Window</button>
      `;
    }
  } else if (selectedStudioEntity.type === 'label') {
    title.innerText = '🏷️ Room Label';
    actionButtons = `
      <button class="btn-hud" onclick="renameSelectedLabel()">✏️ Rename Label</button>
      <button class="btn-hud" onclick="deleteSelectedStudioEntity()" style="color:#f87171;">🗑️ Delete Label</button>
    `;
  } else if (selectedStudioEntity.type === 'wall') {
    const w = studioLines[selectedStudioEntity.index];
    const wLen = Math.round((Math.hypot(w.x2 - w.x1, w.y2 - w.y1) / scalePixelsPerMeter) * 1000);
    title.innerText = w.isCavity ? `Cavity (${wLen}mm)` : `Wall (${wLen}mm)`;
    actionButtons = `
      <button class="btn-hud" onclick="stretchSelectedWall()">📐 Stretch Wall Length</button>
      <button class="btn-hud" onclick="calibrateSelectedWall()">🌐 Calibrate Plan Scale</button>
      <button class="btn-hud" onclick="toggleWallCavity()">🧱 Toggle Cavity Wall</button>
      <button class="btn-hud" onclick="deleteSelectedStudioEntity()" style="color:#f87171;">🗑️ Delete Wall</button>
    `;
  }
...
function stretchSelectedWall() {
  if (!selectedStudioEntity || selectedStudioEntity.type !== 'wall') return;
  const w = studioLines[selectedStudioEntity.index];
  const currentPx = Math.hypot(w.x2 - w.x1, w.y2 - w.y1);
  const currentMm = Math.round((currentPx / scalePixelsPerMeter) * 1000);

  const input = prompt(`Current wall length: ${currentMm}mm\n\nEnter new physical length for this wall in millimeters (e.g. 2400):`, currentMm);
  if (!input) return;

  const newMm = parseFloat(input);
  if (newMm > 0 && currentPx > 0) {
    recordStudioState();
    const targetPx = (newMm / 1000) * scalePixelsPerMeter;
    const ratio = targetPx / currentPx;

    const oldX2 = w.x2;
    const oldY2 = w.y2;
    const newX2 = w.x1 + (w.x2 - w.x1) * ratio;
    const newY2 = w.y1 + (w.y2 - w.y1) * ratio;

    const dx = newX2 - oldX2;
    const dy = newY2 - oldY2;

    const isThisHoriz = Math.abs(w.y2 - w.y1) < 2;
    const isThisVert = Math.abs(w.x2 - w.x1) < 2;

    // Shift connected walls while keeping perpendicular walls square
    studioLines.forEach((other, oIdx) => {
      if (oIdx === selectedStudioEntity.index) return;

      const connAtStart = Math.hypot(other.x1 - oldX2, other.y1 - oldY2) < 1;
      const connAtEnd = Math.hypot(other.x2 - oldX2, other.y2 - oldY2) < 1;

      if (!connAtStart && !connAtEnd) return;

      const isOtherVert = Math.abs(other.x2 - other.x1) < 2;
      const isOtherHoriz = Math.abs(other.y2 - other.y1) < 2;

      // If this wall is horizontal and connected is vertical, slide entire vertical wall
      if (isThisHoriz && isOtherVert) {
        other.x1 += dx;
        other.x2 += dx;
      }
      // If this wall is vertical and connected is horizontal, slide entire horizontal wall
      else if (isThisVert && isOtherHoriz) {
        other.y1 += dy;
        other.y2 += dy;
      }
      // Otherwise adjust only the joint endpoint
      else {
        if (connAtStart) { other.x1 = newX2; other.y1 = newY2; }
        if (connAtEnd) { other.x2 = newX2; other.y2 = newY2; }
      }
    });
...
function calibrateWallAt(x, y) {
  ...
  const currentMm = Math.round((pxLen / scalePixelsPerMeter) * 1000);
  const realMmInput = prompt(`Current wall length: ${currentMm}mm\n\nEnter true physical length for this wall in millimeters to calibrate entire plan (e.g. 2400 or 6000):`, currentMm);
  if (realMmInput) {
    const enteredMm = parseFloat(realMmInput);
    if (enteredMm > 0) {
      recordStudioState();
      scalePixelsPerMeter = pxLen / (enteredMm / 1000);
...
function renderStudio() {
  ...
  studioLines.forEach((l, idx) => {
    ...
    if (isSel) {
      // Draw interactive corner vertex handles for stretching
      const handleR = 7 / studioScale;
      [{ x: l.x1, y: l.y1 }, { x: l.x2, y: l.y2 }].forEach(pt => {
        studioCtx.fillStyle = '#0284c7';
        studioCtx.strokeStyle = '#ffffff';
        studioCtx.lineWidth = 2 / studioScale;
        studioCtx.beginPath();
        studioCtx.arc(pt.x, pt.y, handleR, 0, Math.PI * 2);
        studioCtx.fill();
        studioCtx.stroke();
      });
    }
  ...
  // Smart CAD Leveling & Alignment Extension Guides
  if (isStudioDrawing && studioAlignGuides.length > 0) {
    studioCtx.save();
    studioCtx.strokeStyle = '#06b6d4';
    studioCtx.lineWidth = 1.2 / studioScale;
    studioCtx.setLineDash([5 / studioScale, 3 / studioScale]);

    // Render guidelines clamped strictly to visible viewport boundaries
    const viewLeft = -studioPanX / studioScale;
    const viewRight = (w - studioPanX) / studioScale;
    const viewTop = -studioPanY / studioScale;
    const viewBottom = (h - studioPanY) / studioScale;

    studioAlignGuides.forEach(g => {
      studioCtx.beginPath();
      if (g.type === 'horizontal') {
        studioCtx.moveTo(viewLeft, g.y);
        studioCtx.lineTo(viewRight, g.y);
      } else if (g.type === 'vertical') {
        studioCtx.moveTo(g.x, viewTop);
        studioCtx.lineTo(g.x, viewBottom);
      }
      studioCtx.stroke();

      // Highlight the anchor corner you are aligning with
      studioCtx.fillStyle = '#06b6d4';
      studioCtx.beginPath();
      studioCtx.arc(g.sourceX, g.sourceY, 4 / studioScale, 0, Math.PI * 2);
      studioCtx.fill();
    });
    studioCtx.restore();
  }
